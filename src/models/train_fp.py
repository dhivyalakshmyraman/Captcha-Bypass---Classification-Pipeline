"""Train fingerprint sub-model (fp_model).
...
from src.features.fp_pipeline import run_pipeline as run_fp_pipeline, FEATURE_NAMES
...
def train(data_dir: Path, processed_dir: Path, mlflow_uri: str) -> None:
    ...
    mlflow.set_experiment("fp_detection")

    features_path = processed_dir / "fp_features.parquet"
    if features_path.exists():
        features = pd.read_parquet(features_path)
        print(f"Loaded existing features: {len(features)} rows")
    else:
        features = run_fp_pipeline(data_dir, processed_dir)
    ...
    with mlflow.start_run(run_name="fp_model_v1"):
        params = {
        ...
        cm_path = processed_dir / "fp_confusion_matrix.txt"
        with open(cm_path, "w") as f:
            f.write(f"ROC-AUC: {roc_auc:.4f}\nF1: {f1:.4f}\n\n{cm}")
        mlflow.log_artifact(str(cm_path))

        pipeline = Pipeline([("scaler", scaler), ("model", calibrated_model)])
        mlflow.sklearn.log_model(pipeline, "fp_model")

        pipeline_path = processed_dir / "fp_pipeline.joblib"
        joblib.dump(pipeline, pipeline_path)

        test_df = pd.DataFrame(X_test, columns=FEATURE_NAMES)
        test_df["label"] = y_test
        test_path = processed_dir / "fp_test.parquet"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train fingerprint model")
