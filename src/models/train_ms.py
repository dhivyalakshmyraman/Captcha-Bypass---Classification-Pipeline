"""Train mouse dynamics sub-model (ms_model).
...
from src.features.ms_pipeline import run_pipeline as run_ms_pipeline, FEATURE_NAMES


def train(data_dir: Path, processed_dir: Path, mlflow_uri: str) -> None:
    """Train the mouse dynamics model."""
    mlflow_uri_formatted = f"file:///{Path(mlflow_uri).resolve().as_posix()}"
    mlflow.set_tracking_uri(mlflow_uri_formatted)
    mlflow.set_experiment("ms_detection")

    features_path = processed_dir / "ms_features.parquet"
    if features_path.exists():
        features = pd.read_parquet(features_path)
        print(f"Loaded existing features: {len(features)} rows")
    else:
        features = run_ms_pipeline(data_dir, processed_dir)
    ...
    with mlflow.start_run(run_name="ms_model_v1"):
        params = {
        ...
        cm_path = processed_dir / "ms_confusion_matrix.txt"
        with open(cm_path, "w") as f:
            f.write(f"ROC-AUC: {roc_auc:.4f}\nF1: {f1:.4f}\n\n{cm}")
        mlflow.log_artifact(str(cm_path))

        pipeline = Pipeline([("scaler", scaler), ("model", calibrated_model)])
        mlflow.sklearn.log_model(pipeline, "ms_model")

        pipeline_path = processed_dir / "ms_pipeline.joblib"
        joblib.dump(pipeline, pipeline_path)

        test_df = pd.DataFrame(X_test, columns=FEATURE_NAMES)
        test_df["label"] = y_test
        test_path = processed_dir / "ms_test.parquet"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train mouse dynamics model")
